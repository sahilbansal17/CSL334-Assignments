...
void str_cli(FILE *fp, int sockfd) {
    ...
    for ( ; ; ) { 
        ...
        printf("Enter any character to get the daytime : "); 
        n = 0; 
...
int main(int argc, char **argv) {
	...
	if (show_server_details(servaddr) == 1) {
		...
		if ((sendto(sockfd, (const char *)hello, strlen(hello), MSG_CONFIRM, (const struct sockaddr *) &servaddr, sizeof(servaddr))) == -1) {
