...
int main(int argc, char **argv) {
	...
	// concurrent server
	// multiple clients requests can be processed at the same time
	for ( ; ; ) {
		...
		if ((childpid = fork()) == 0) {
			...
			close(listenfd); // child closes listening descriptor
			str_daytime(connfd);
			exit(0);
		}
		close(connfd); // parent closes connected (accepted) socket
	}
